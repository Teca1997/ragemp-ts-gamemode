...
let scenarionIndex = 0;

mp.events.add('openScenarioBrowser', () => {
	if (browserOpen) {
		browserOpen = false;
	...
	browserOpen = true;
	scenarionIndex = 0;
	mp.players.local.taskStartScenarioInPlace(scenarios[scenarionIndex], 0, true);
});
...
mp.keys.bind(0x25, true, function () {
	...
	scenarionIndex -= 1;
	if (scenarionIndex == -1) {
		scenarionIndex = scenarios.length - 1;
	}
	mp.players.local.clearTasksImmediately();
	mp.players.local.taskStartScenarioInPlace(scenarios[scenarionIndex], 0, true);
	mp.gui.chat.push('scenarion index ' + scenarionIndex);
});
...
mp.keys.bind(0x27, true, function () {
	...
	scenarionIndex += 1;
	if (scenarionIndex == scenarios.length) {
		scenarionIndex = 0;
	}
	mp.players.local.clearTasksImmediately();
	mp.players.local.taskStartScenarioInPlace(scenarios[scenarionIndex], 0, true);
...
mp.events.add('render', () => {
	if (browserOpen) {
		mp.game.graphics.drawText(`Current scenarion: ${scenarios[scenarionIndex]}`, [0.5, 0.065], {
			font: 0,
