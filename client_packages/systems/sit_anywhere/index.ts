import 'systems/sit_anywhere/scenarionBrowser';

import { rayc } from 'systems/raycasting';
import { sittableObjects } from 'systems/sit_anywhere/config';
...
function getShortesDistanceBetweenNCoordiates(startCoord: Vector3, coords: { vector: Vector3; headingOffset: number }[]): { vector: Vector3; headingOffset: number } {
	let distance = 5;
	let closestCoord: { vector: Vector3; headingOffset: number } = { vector: new mp.Vector3(0, 0, 0), headingOffset: 180 };
	coords.forEach((coord) => {
		const coordsDistance = mp.game.gameplay.getDistanceBetweenCoords(startCoord.x, startCoord.y, startCoord.z, coord.vector.x, coord.vector.y, coord.vector.z, true);
...
mp.events.add('sit:sit', () => {
	...
	const heading: number = mp.game.invokeFloat(RageEnums.Natives.ENTITY.GET_ENTITY_HEADING, target.entity.handle == undefined ? target.entity : target.entity.handle) + currentSeat.headingOffset;
