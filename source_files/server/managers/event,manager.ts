		...
		for (const module of modules) {
			for (const provider of module.providers.values()) {
				const instance: EventInstance =
					provider.instance as EventInstance;
				...
				if (eventMetadata) {
					for (const {
						...
						const method =
							instance[propertyKey as keyof typeof instance];

						mp.events[proc == true ? 'addProc' : 'add'](
							eventName,
						...
						console.log(
							`${'[EVENT MANAGER]'} Registered ${
								proc == true ? 'proc' : ''
							}event ${eventName} on method ${
								method.name
							} from class ${instance.constructor.name}`
						);
					}
				}
			}
