import { NPCDriverModels } from './npc_driver_models';
import { NPCVehicleModels } from './npc_vehicle_models';
import { NPC_Vehicle_Parking_Spots } from './npc_vehicle_parking_spots';

const random = (min: number, max: number) => Math.floor(Math.random() * (max - min)) + min;
const MAX_Controler_Distance: number = 400;
const MAX_NPC_PER_PLAYER: number = 50;
			...
			mp.players.forEach(async (player: PlayerMp) => {
				if (NPC_LIST[player.id].Active_NPC != null) {
					...
					if (NPC_LIST[player.id].Active_NPC.length <= MAX_NPC_PER_PLAYER) {
						...
						let phash = random(0, NPCDriverModels.length - 1);
						let vhash = random(0, NPCVehicleModels.length - 1);
						...
						for (let i = 0; i < newlist.length; i++) {
							...
							if (newlist[posrnd].spawnable < Date.now() && numPlayersInRange == 0 && mp.peds.toArray().find((x) => x.dist(newlist[posrnd].position) < 5) == null) {
								...
								let ped: PedMp = mp.peds.new(NPCDriverModels[phash], getPedOffset(newlist[posrnd].position), { heading: 0, dynamic: true, lockController: false, dimension: 0 });
								...
								let veh: VehicleMp = mp.vehicles.new(NPCVehicleModels[vhash], newlist[posrnd].position, {
								...
								ped.setVariables({ blip: blip.id, vehicle: veh.id, traffic: true });
...
function NPC_ExitStream(player: PlayerMp, pedId: number) {
	...
	let vehId = NPC.getVariable('vehicle');
