console.log('componentSync started');
// "Borrowed" from attachments resource, credits to ragempdev
function serializeComponentSet(dataSet: any) {
	return Array.from(dataSet)
		.map((hash: any) => hash.toString(36))
		.join('|');
}
 ...
 */
mp.Player.prototype.giveWeaponComponent = function (weaponHash: number, componentHash: number) {
	...
	console.log(this.__weaponComponents);
};
 ...
 * @throws {TypeError} If any of the arguments is not a number.
 */
mp.Player.prototype.hasWeaponComponent = function (weaponHash: number, componentHash: number) {
 ...
 */
mp.Player.prototype.getWeaponComponents = function (weaponHash: number): number[] {
 ...
 * @throws {TypeError} If any of the arguments is not a number.
 */
mp.Player.prototype.removeWeaponComponent = function (weaponHash: number, componentHash: number) {
 ...
 * @throws {TypeError} If weaponHash argument is not a number.
 */
mp.Player.prototype.removeAllWeaponComponents = function (weaponHash: number) {
...
mp.events.add('playerWeaponChange', (player, _oldWeapon, newWeapon) => {
	player.setVariable(
		'currentWeaponComponents',
		newWeapon.toString(36) + '.' + (player.__weaponComponents.hasOwnProperty(newWeapon) ? serializeComponentSet(player.__weaponComponents[newWeapon]) : '')
	);
});
